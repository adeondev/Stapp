...
async fn ler_gravado(dir: &std::path::Path, user_id: &str) -> image::DynamicImage {
    image::load_from_memory(&read(dir, user_id).await.expect("o arquivo tinha que existir")).unwrap()
}

#[tokio::test]
async fn qualquer_tamanho_vira_um_quadrado_de_256() {
    let dir = TestDir::new();
    store(dir.path(), "u1", &png(800, 400)).await.unwrap();
...
#[tokio::test]
async fn o_que_nao_e_imagem_e_recusado() {
    let dir = TestDir::new();
    ...
    assert!(matches!(
        store(dir.path(), "u1", &mentiroso).await,
        Err(AvatarError::NaoEImagem)
    ));
    assert!(matches!(
        store(dir.path(), "u1", b"texto puro").await,
        Err(AvatarError::NaoEImagem)
    ));
    assert!(
        read(dir.path(), "u1").await.is_none(),
        "nada pode ter sido gravado"
    );
}

#[tokio::test]
async fn grava_sempre_em_webp_qualquer_que_seja_a_entrada() {
    let dir = TestDir::new();
    store(dir.path(), "u1", &png(300, 300)).await.unwrap();

    let bytes = read(dir.path(), "u1").await.unwrap();
    assert_eq!(
...
#[tokio::test]
async fn subir_de_novo_sobrescreve_em_vez_de_acumular() {
    let dir = TestDir::new();
    store(dir.path(), "u1", &png(300, 300)).await.unwrap();
    store(dir.path(), "u1", &png(120, 120)).await.unwrap();
...
#[tokio::test]
async fn remover_apaga_e_nao_reclama_se_ja_nao_existe() {
    let dir = TestDir::new();
    store(dir.path(), "u1", &png(300, 300)).await.unwrap();
