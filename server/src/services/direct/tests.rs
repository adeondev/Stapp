...
async fn a_mensagem_chega_so_nas_duas_pontas() {
    ...
    send(&server.state, "d1", alice.id.clone(), "  oi alice  ", Vec::new(), None).await;
...
async fn cada_lado_recebe_o_outro_como_dono_da_conversa() {
    ...
    let mut events = server.state.subscribe();
    send(&server.state, "d1", alice.id.clone(), "oi", Vec::new(), None).await;
...
async fn nao_lidas_acumulam_e_zeram_ao_abrir() {
    ...
    send(&server.state, "d1", alice.id.clone(), "uma", Vec::new(), None).await;
    send(&server.state, "d1", alice.id.clone(), "duas", Vec::new(), None).await;
...
async fn conversa_offline_espera_no_historico() {
    ...
    send(&server.state, "d1", alice.id.clone(), "te procurei", Vec::new(), None).await;
...
async fn a_lista_traz_com_quem_voce_falou_e_quantas_faltam_ler() {
    ...
    send(&server.state, "d1", alice.id.clone(), "primeira", Vec::new(), None).await;
...
async fn recusa_conversa_consigo_mesmo_e_conta_inexistente() {
    ...
    let mut events = server.state.subscribe();
    send(&server.state, "d1", daniel.id.clone(), "eco", Vec::new(), None).await;
    send(&server.state, "d1", "nao-existe".to_string(), "oi", Vec::new(), None).await;
...
async fn ler_numa_sessao_limpa_o_badge_das_outras() {
    ...
    send(&server.state, "d1", alice.id.clone(), "olha isso", Vec::new(), None).await;
