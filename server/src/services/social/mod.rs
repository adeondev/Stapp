...
pub async fn send_snapshot(state: &AppState, user_id: &UserId) {
    let allow_member_dms = state.db.allow_member_dms(user_id).await.unwrap_or(true);
    let members = match state.db.social_records(user_id).await {
        Ok(records) => records
            .into_iter()
...
pub async fn request(state: &AppState, peer_id: &str, other: UserId) {
    ...
    if !available(state, &me.user_id, &other).await {
        return refuse(state, peer_id);
    }
    match state.db.request_friend(&me.user_id, &other).await {
...
pub async fn accept(state: &AppState, peer_id: &str, other: UserId) {
    ...
    mutate_pair(state, peer_id, &me.user_id, &other, || {
        state.db.accept_friend(&me.user_id, &other)
    })
    .await;
}
...
pub async fn decline(state: &AppState, peer_id: &str, other: UserId) {
    ...
    mutate_pair(state, peer_id, &me.user_id, &other, || {
        state.db.delete_friend_request(&other, &me.user_id)
    })
    .await;
}
...
pub async fn cancel(state: &AppState, peer_id: &str, other: UserId) {
    ...
    mutate_pair(state, peer_id, &me.user_id, &other, || {
        state.db.delete_friend_request(&me.user_id, &other)
    })
    .await;
}
...
pub async fn remove(state: &AppState, peer_id: &str, other: UserId) {
    ...
    mutate_pair(state, peer_id, &me.user_id, &other, || {
        state.db.remove_friend(&me.user_id, &other)
    })
    .await;
}
...
pub async fn block(state: &AppState, peer_id: &str, other: UserId) {
    ...
    match state.db.block_user(&me.user_id, &other).await {
        Ok(_) => {
...
pub async fn unblock(state: &AppState, peer_id: &str, other: UserId) {
    ...
    mutate_pair(state, peer_id, &me.user_id, &other, || {
        state.db.unblock_user(&me.user_id, &other)
    })
    .await;
}
...
pub async fn update_privacy(state: &AppState, peer_id: &str, allow: bool) {
    ...
    if let Err(error) = state.db.set_allow_member_dms(&me.user_id, allow).await {
...
async fn mutate_pair<F, Fut>(state: &AppState, peer_id: &str, me: &UserId, other: &UserId, operation: F)
where
    F: FnOnce() -> Fut,
    Fut: std::future::Future<Output = anyhow::Result<bool>>,
{
    if !available(state, me, other).await {
        return refuse(state, peer_id);
    }
...
async fn available(state: &AppState, me: &UserId, other: &UserId) -> bool {
    me != other
        && state
            .db
            .account_by_id(other)
