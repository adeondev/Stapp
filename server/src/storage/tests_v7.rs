...
fn localiza_mensagem_de_canal_e_de_conversa_pelo_id() {
    ...
    db.insert(&mensagem(&daniel, "m1", "no canal", 100)).unwrap();
...
fn o_historico_traz_reacao_de_todas_as_mensagens_do_lote() {
    ...
    let com_reacao: Vec<_> = historico.iter().filter(|m| !m.reactions.is_empty()).collect();
...
fn responder_mensagem_apagada_deixa_a_previa_sem_autor() {
    ...
    db.delete_message_cascade("alvo", &daniel.id).unwrap().unwrap();
...
fn so_o_autor_edita_e_apaga_a_propria_mensagem() {
    ...
    let daniel = conta(&db, "daniel");
    let alice = conta(&db, "alice");
    db.insert(&mensagem(&daniel, "m1", "original", 100)).unwrap();

    assert!(
        !db.update_message_text("m1", &alice.id, "invadido", &[], false, 200)
            .unwrap()
    );
    assert!(db.delete_message_cascade("m1", &alice.id).unwrap().is_none());
...
fn apagar_mensagem_leva_junto_anexo_reacao_e_enquete() {
    ...
    let db = Db::open(&dir.database()).unwrap();
    let daniel = conta(&db, "daniel");
    db.insert(&mensagem(&daniel, "m1", "com tudo", 100)).unwrap();

    db.insert_attachment("a1", &daniel.id, "foto.png", "image/png", 10, "uploads/a1", 1)
        .unwrap();
    ...
    let chaves = db.delete_message_cascade("m1", &daniel.id).unwrap().unwrap();
